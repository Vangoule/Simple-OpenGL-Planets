...
void Model::Setup(std::string fName, const char *cubeFaces[6])
{
	fileName = fName;
	cubeMap = true;
...
*/	glGenBuffers(1, &vertexbuffer);
	...
	cubeTexture = engine->getGraphicsManager()->loadCubeMapTexture(cubeFaces);
...
void Model::Render()
{
	...
	if(cubeMap == false){
		...
		glUniform1i(engine->getGraphicsManager()->getBoolCubeMapID(), 0);
	}else{
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_CUBE_MAP, cubeTexture); 
		glUniform1i(engine->getGraphicsManager()->getCubeMapTextureID(), 0);
		glUniform1i(engine->getGraphicsManager()->getBoolCubeMapID(), 1);
	}
